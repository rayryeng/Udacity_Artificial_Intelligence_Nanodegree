...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        # For each effect in actionA...
        for effectA in actionA.effects:
            # If the negative of this is in actionB, then inconsistent
            if ~effectA in actionB.effects:
                return True
        
        # Repeat the same for actionB
...
#        raise NotImplementedError
        ...
        # Similar logic to previous method, but we search within preconditions instead
        for effectA in actionA.effects:
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        for i, l in enumerate(self.literal_layers):
            for g in self.goal:
                if g not in goal_cost and g in l:
                    goal_cost[g] = i
        return sum(goal_cost.values())
